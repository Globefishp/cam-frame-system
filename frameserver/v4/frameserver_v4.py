# frameserver_v4.py
# Haiyun Huang with Google Gemini 3.1 Pro (2607xx?)
# V4 add additional control of consumer `get`.
...
class FrameServer:
    ...
    def resume(self) -> None:
        """
        Resume the frameserver.
        """
        pass

    def reset(self) -> int:
        """
        Clear and reset the buffer when frameserver is paused. 
        Will block until all sync ticket is released by `release_sync()`,
        and then the buffer will be cleared and reset.
    ...
    def unlink(self) -> None:
        ...
        Raise:
            FileNotFoundError: If the metadata shared memory segment cannot be found.
            RuntimeError: If any unexpected error occurs.
        """
        if self._shm:
            try:
