...
import time
from ringbuffers.shared_ring_buffer_v3a import ProcessSafeSharedRingBuffer
...
def test_peek_frames_basic(shared_buffer):
    """Test basic peek_frames operations (specifically peek_frames(0, 1))."""
    # 1. Peek empty buffer
    assert shared_buffer.peek_frames(0, 1) is None, "Peek on empty buffer should return None"

    # 2. Put one frame and peek
    frame0 = create_dummy_frames(0, 1)
    assert shared_buffer.put(frame0) is True
    peeked0_frames = shared_buffer.peek_frames(0, 1)
    assert peeked0_frames is not None, "Peek after first put returned None"
    assert len(peeked0_frames) == 1, "Peek after first put returned incorrect number of frames"
    peeked0 = peeked0_frames[0]
    ...
    peeked0_again_frames = shared_buffer.peek_frames(0, 1) # Peek again
    assert peeked0_again_frames is not None
    assert len(peeked0_again_frames) == 1
    peeked0_again = peeked0_again_frames[0]
    ...
    frame1 = create_dummy_frames(1, 1)
    assert shared_buffer.put(frame1) is True
    peeked1_frames = shared_buffer.peek_frames(0, 1)
    assert peeked1_frames is not None, "Peek after second put returned None"
    assert len(peeked1_frames) == 1
    peeked1 = peeked1_frames[0]
    ...
    with shared_buffer.pointer_lock:
        read_ptr_before, write_ptr_before, occupied_before, last_get_before, protected_before, eager_before = shared_buffer._get_pointers_metadata()

    shared_buffer.peek_frames(0, 1) # Perform the peek

    with shared_buffer.pointer_lock:
        read_ptr_after, write_ptr_after, occupied_after, last_get_after, protected_after, eager_after = shared_buffer._get_pointers_metadata()
    ...
    assert protected_before == protected_after, "Protected count changed after peek"
    assert eager_before == eager_after, "Eager release count changed after peek"


def test_peek_frames_after_get_no_release(shared_buffer):
    ...
    peeked_after_get2_list = shared_buffer.peek_frames(0, 1)
    assert peeked_after_get2_list is not None, "Peek after get(2) returned None"
    assert len(peeked_after_get2_list) == 1
    peeked_after_get2 = peeked_after_get2_list[0]
    ...
    peeked_after_get1_list = shared_buffer.peek_frames(0, 1)
    assert peeked_after_get1_list is not None, "Peek after get(1) returned None"
    assert len(peeked_after_get1_list) == 1
    peeked_after_get1 = peeked_after_get1_list[0]
...
def test_peek_frames_after_get_and_release(shared_buffer):
    ...
    peeked_list = shared_buffer.peek_frames(0, 1)
    assert peeked_list is not None, "Peek after explicit release returned None"
    assert len(peeked_list) == 1
    peeked = peeked_list[0]
    assert np.array_equal(peeked, frames_initial[2]), "Peek after explicit release should return last PUT frame (F12)"


def test_peek_frames_wrap_around(shared_buffer):
    ...
    # Peek - should get F6
    peeked_list = shared_buffer.peek_frames(0, 1)
    assert peeked_list is not None, "Peek after wrap returned None"
    assert len(peeked_list) == 1
    peeked = peeked_list[0]
...
def _concurrent_peek_task(buffer_or_source, duration, results_queue, stop_event, use_process=False):
    ...
    try:
        ...
        while not stop_event.is_set() and time.time() - start_time < duration:
            peeked_frames = buffer.peek_frames(0, 1)
            if peeked_frames is not None and len(peeked_frames) > 0:
                peeked_frame = peeked_frames[0]
                ...
                peeked_values.append(None) # Record None if buffer was empty or peek failed
            time.sleep(0.001) # Peek frequently but avoid busy-waiting
...
def test_peek_frames_concurrent(shared_buffer):
    """
    Tests peek_frames(0, 1) concurrently with put and get operations using processes.
    """
