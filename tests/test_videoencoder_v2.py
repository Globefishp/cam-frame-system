...
class DummyValidEncoder(BaseVideoEncoder):
    ...
    """
    def __init__(self, shared_buffer, output_path, batch_size, mp_record_dict, **kwargs):
        super().__init__(shared_buffer, output_path, batch_size, **kwargs)
        self.mp_record_dict = mp_record_dict

    def _initialize_encoder(self, logger):
...
class DummyErrorEncoder(DummyValidEncoder):
    ...
    def _initialize_encoder(self, logger):
        ...
        super()._initialize_encoder(logger)
...
def test_normal_lifecycle_thread_producer(ring_buffer, record_dict):
    """
    验证要点 2：线程生产者情况
    - 启动后、写入前，init 已调用。
    - 写入非整数倍 batch 的帧，stop() 后数据被完整处理。
    - uninit 被调用。
    - buffer 未销毁，再次 start() 能继续处理新写入的数据。
    """
    encoder = DummyValidEncoder(
        shared_buffer=ring_buffer, output_path="dummy.mp4", 
    ...
    # 2. 线程生产者：写入 7 帧 (batch_size=3, 不是整数倍)
    def producer_1():
        frames = generate_frames(0, 7)
        ring_buffer.put(frames)
    ...
    frames2 = generate_frames(7, 4)
    ring_buffer.put(frames2)

    # 5. 再次启动并停止
    encoder = DummyValidEncoder( # 注意：原代码注释说明需要 Recreate instance
        shared_buffer=ring_buffer, output_path="dummy2.mp4", 
        batch_size=3, mp_record_dict=record_dict
    )
    encoder.start()
    time.sleep(0.5)
...
def process_producer(buf):
    # 注意：这里需要传入 buf 实例（v2a 支持直接跨进程传实例）
    frames = generate_frames(0, 5)
    buf.put(frames)

def test_normal_lifecycle_process_producer(ring_buffer, record_dict):
    """
    验证要点 2：多进程生产者情况
    利用分离的进程对 RingBuffer 写入，验证跨进程同步是否健壮。
    """
    encoder = DummyValidEncoder(
        shared_buffer=ring_buffer, output_path="dummy.mp4", 
        batch_size=4, mp_record_dict=record_dict
    )
    encoder.start()

    p = mp.Process(target=process_producer, args=(ring_buffer,))
    p.start()
    p.join()
    ...
    assert record_dict['init_calls'] == 1
    assert record_dict['uninit_calls'] == 1
    assert list(record_dict['encoded_frames']) == [0, 1, 2, 3, 4]


def test_exception_handling_encode_error(ring_buffer, record_dict):
    """
    验证要点 3：编码过程中抛出业务异常 (EncoderException)
    期望：进程捕获异常后触发 Eager Stop (不继续消费)，调用 uninit 然后退出。
    """
    encoder = DummyErrorEncoder(
        error_type='encode', shared_buffer=ring_buffer, 
        output_path="dummy.mp4", batch_size=2, mp_record_dict=record_dict
    )
    encoder.start()

    # 写入 6 帧，在第 4 帧(idx=3) 会抛出异常
    ring_buffer.put(generate_frames(0, 6))
...
def test_exception_handling_init_error(ring_buffer, record_dict):
    """
    验证要点 3：初始化失败
    期望：主进程 start() 时等待 ready 超时，主动抛出 TimeoutError，并且不发生死锁。
    """
    encoder = DummyErrorEncoder(
        error_type='init', shared_buffer=ring_buffer, 
        output_path="dummy.mp4", batch_size=2, mp_record_dict=record_dict
    )
