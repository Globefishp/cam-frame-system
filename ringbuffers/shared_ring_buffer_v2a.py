...
class ProcessSafeSharedRingBuffer:
    ...
    def __init__(self,
        ...
        Deprecated:
            This attach mode is deprecated. Pass the buffer object directly to the
            subprocess (e.g. via mp.Process args). The pickle protocol (__getstate__/
            __setstate__) will handle re-attachment automatically. This mode is kept
    ...
    def __init__(self,
        ...
        Deprecated:
            The create=False (attach) mode is deprecated. Prefer passing the buffer
            directly to a subprocess; __getstate__/__setstate__ handles re-attachment
            automatically via pickle. This mode is kept for backward compatibility.
        """
        ...
        if create:
            ...
            # --- Attach Mode (Deprecated) ---
            # Deprecated: Use pickle protocol instead. Pass the buffer object directly
            # to subprocess args; __getstate__/__setstate__ handles re-attachment.
    ...
    def __getstate__(self) -> dict:
        ...
        memory binding is introduced):
            ...
            _metadata_ctypes : ctypes.Structure mapped via Metadata.from_buffer(shm.buf).
                               Binds to this process's virtual address. Always rebuilt
                               in __setstate__ from the re-attached shm.buf.

            _metadata_shm    : multiprocessing.shared_memory.SharedMemory object.
                               Replaced with its .name string so the child process can
                               re-attach by name (SharedMemory(name=...)).
    ...
    def __setstate__(self, state: dict):
        ...
        Restores all pure Python attributes via __dict__.update, then re-attaches
        ...
        child process, enabling transparent DI injection:
            ...
            # mp.Process pickles 'buffer' → __getstate__ → transfer →
            # __setstate__ in child → buffer is fully functional, no create=False needed
        """
        ...
        if metadata_name is not None:
            ...
            # 3. Rebuild the ctypes mapping bound to THIS process's virtual address.
            self._metadata_ctypes = Metadata.from_buffer(self._metadata_shm.buf)
        else:
            self._metadata_shm = None
            self._metadata_ctypes = None

        if data_name is not None:
            self._data_shm = mp_shm.SharedMemory(name=data_name)
        else:
            self._data_shm = None
