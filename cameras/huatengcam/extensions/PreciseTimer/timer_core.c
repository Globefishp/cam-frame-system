...
static unsigned __stdcall timer_loop(void* arg) {
    ...
    while (!state->stop_flag) {
        ...
        // --- 3. 直接调用C函数指针 ---
        uint16_t trigger_result = 0;
        if (state->debug_print == 1) {
            printf("[timer_loop] starting trigger...\n");
        }
        int ret = state->trigger_func(state->hCamera, &trigger_result);
        if (state->debug_print == 1) {
            printf("[timer_loop] ret: %x, trigger_result: %d\n", ret, trigger_result);
        }
...
TimerState* timer_core_create(
    ...
    if (state->debug_print == 1) {
        printf("[timer_core_create] interval_ns: %lld, busy_wait_ns: %d, priority: %d, debug_print: %d\n", interval_ns, busy_wait_ns, priority, debug_print);
        printf("[timer_core_create] hCamera: %p, trigger_func: %p\n", hCamera, trigger_func);
    }
