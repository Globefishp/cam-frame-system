...
from . import PreciseTimer
from ...pco_edge_42 import PCOEdge42Camera
...
# By test: although fast px rate, the soft trigger can only reach 36 fps

# --- Main Application ---
# 1. Initialize camera in Python to get the handle
mycam = PCOEdge42Camera()
mycam.open()
mycam.hw_trg_in=True
mycam.fast_px_rate=True
mycam.start_capture()
mycam.force_trigger()
image = mycam.grab_extended_info()
print(image.shape)

mycam_handle = mycam.c_handle
print(mycam_handle)
c_trigger_func = mycam.c_force_trigger

try: 
    # 2. Instantiate our new Cython C-function timer
    interval_s = 1.0 / FRAME_RATE
    timer = PreciseTimer.PrecisionTimer(
        interval_s=interval_s,
        c_trigger_func=c_trigger_func, # Pass the ctypes function object
        hCamera=mycam_handle,
    ...
    while time.time() - start_time < TEST_DURATION:
        ...
        try:
            img = mycam.grab_extended_info() # This should ideally just fetch the latest 
            timestamp = mycam.extract_extended_info(img)['timestamp']
            if img is not None:
                frame_count += 1
                imgs.append(img)
                timestamps_cam.append(timestamp * 1_000_000_000 // mycam.hw_timecode_timebase) # convert camera timestamp to ns.
                timestamps_python.append(time.perf_counter_ns()) # convert python timestamp to ns.
                if frame_count % 10 == 0:
                    print(f'Frame: {frame_count}, timestamp:{timestamps_cam[-1]:.3f}, fps: {10 * 1_000_000_000 / (timestamps_cam[-1] - last_time):.2f}')
                    last_time = timestamp * 1_000_000_000 // mycam.hw_timecode_timebase
            else:
                print('NullFrame')
