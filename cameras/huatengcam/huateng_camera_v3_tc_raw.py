...
# V3: The lastest version, using C extension to do soft trigger and unpack raw. update fps related set & stats.
...
class Camera(object):
    ...
    """
    # TODO: Camera properties should be classified into private and public two groups.
    # Currently it's a little bit messy.
    def __init__(self,
                 DevInfo,
                 exposure_time_ms: float = 10.0,
                 trigger_mode: str = 'freerun',  # 'freerun' or 'soft_trigger'
                 fps: Optional[float] = None,    # Required for soft_trigger mode
                 gain: float = GAIN,
                 tc: bool = False,
                 hibitdepth: int = 0,
                 **kwargs):
        ...
        self.timer = None # For soft trigger
        ...
        self._ema_alpha = 0.1  # 平滑因子，值越小越平滑
    ...
    def target_fps(self) -> float:
        ...
        if self.trigger_mode == 'soft_trigger':
            if self._target_fps is None:
                raise ValueError("target_fps is not set in soft_trigger mode.")
    ...
    def actual_fps(self) -> float:
        """根据相机模式返回实际FPS。"""
        if self.hCamera == 0:
            raise ValueError("Cannot get actual_fps before camera is opened.")

        if self.trigger_mode == 'soft_trigger':
            # 在软触发模式下，实际FPS由外部决定，返回目标值作为代理
            if self._target_fps is None:
                raise ValueError("target_fps is not set in soft_trigger mode.")
            return self._target_fps
        else:  # freerun mode
            if self.acutal_exposure_time_ms is not None and self.acutal_exposure_time_ms > 0:
                ...
                raise ValueError("Actual exposure time is not valid.")
            
    @property
    def exposure_time_ms(self) -> float:
        """Gets the actual exposure time from the camera in milliseconds."""
        if self.hCamera > 0:
    ...
    def exposure_time_ms(self, exposure_ms: float):
        """Sets the target exposure time in milliseconds."""
        self._target_exposure_time_ms = exposure_ms
        if self.hCamera > 0:
            if self.trigger_mode == 'soft_trigger' and self._target_fps is not None:
                max_exposure = 1000.0 / self._target_fps
                if exposure_ms > max_exposure:
                    print(f"Warning: Exposure time {exposure_ms}ms is greater than frame interval {max_exposure:.2f}ms. This may cause issues.")
            
            mvsdk.CameraSetExposureTime(self.hCamera, exposure_ms * 1000)
            # Update the actual value after setting
            self.acutal_exposure_time_ms = mvsdk.CameraGetExposureTime(self.hCamera) / 1000.0
    ...
    def _update_grab_fps(self):
        ...
        if self._last_grab_time is not None:
            ...
            if interval > 0:
                ...
                # 应用EMA公式
                self.actual_grab_fps = (current_fps * self._ema_alpha) + \
                                       (self.actual_grab_fps * (1 - self._ema_alpha))
    ...
    def open(self):
        ...
        if self.trigger_mode == 'soft_trigger':
            ...
            # Start the timer to trigger the camera
            interval_s = 1.0 / self._target_fps
            print(f'Camera.open: Starting soft trigger timer of itvl. {interval_s*1000:.3f} ms')
            ...
            mvsdk.CameraSetTriggerMode(hCamera, 0)  # 连续采集
    ...
    def close(self):
        if self.timer is not None:
            print("Camera.close: Stopping timer...")
            self.timer.stop()
            self.timer.join()
            self.timer = None
    ...
    def grab_raw(self, return_timecode: bool = False) -> Union[Optional[np.ndarray], Optional[np.uint32]]:
        '''
        Raw格式输出，可选返回第二个参数为timecode（100μs为单位）
        '''
